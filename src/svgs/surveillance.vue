<template>
<svg version="1.1" id="_x32_" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" xml:space="preserve">
<g>
	<polygon class="st0" points="374.573,52.291 18.91,199.784 133.446,359.567 447.708,229.19 	" style="fill: rgb(75, 75, 75);"></polygon>
	<polygon class="st0" points="0,263.014 40.458,360.869 90.919,340.01 27.556,251.617 	" style="fill: rgb(75, 75, 75);"></polygon>
	<path class="st0" d="M480.533,352.462v41.408h-78.264c-4.411-8.449-13.246-14.232-23.447-14.232c-2.329,0-4.573,0.331-6.725,0.901
		l-36.751-46.424c1.478-3.299,2.315-6.943,2.315-10.791c0-14.611-11.84-26.444-26.444-26.444c-14.605,0-26.451,11.833-26.451,26.444
		c0,14.612,11.846,26.444,26.451,26.444c1.702,0,3.363-0.176,4.966-0.478l37.82,47.76c-1.034,2.828-1.624,5.853-1.624,9.039
		c0,14.612,11.839,26.444,26.444,26.444c10.2,0,19.036-5.776,23.447-14.231h78.264v41.407H512V352.462H480.533z" style="fill: rgb(75, 75, 75);"></path>
</g>
</svg>
</template>
